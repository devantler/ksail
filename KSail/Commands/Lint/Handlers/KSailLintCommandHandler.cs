...
class KSailLintCommandHandler()
{
  internal static async Task<int> HandleAsync(string clusterName, string manifestsPath, CancellationToken token)
  {
    ...
    if (await ValidateYamlAsync(manifestsPath).ConfigureAwait(false) != 0 || await ValidateKustomizationsAsync(clusterName, manifestsPath, token).ConfigureAwait(false) != 0)
    {
      return 1;
    }
    Console.WriteLine("");
  ...
  static async Task<int> ValidateKustomizationsAsync(string clusterName, string manifestsPath, CancellationToken token)
  {
    ...
    {
      if (await KubeconformCLIWrapper.RunAsync(kubeconformFlags, kubeconformConfig, manifest, token).ConfigureAwait(false) != 0)
      {
        Console.WriteLine($"✕ Validation failed for '{manifest}'");
        return 1;
      }
