...
class InitFilesGenerator : IDisposable
{
  ...
  internal async Task GenerateInitFiles(string clusterName, string rootDirectory, CancellationToken token)
  {
    ...
    await GenerateFluxKustomizations(clusterName, fluxSystemDirectory).ConfigureAwait(false);
    await GenerateRepositories(manifestsDirectory).ConfigureAwait(false);
    await GeneratePostBuildVariables(clusterDirectory, clusterName).ConfigureAwait(false);
    await GenerateInfrastructure(manifestsDirectory).ConfigureAwait(false);
    await GenerateApps(manifestsDirectory).ConfigureAwait(false);

    await _k3dGenerator.GenerateK3dConfigAsync($"./{clusterName}-k3d-config.yaml", clusterName).ConfigureAwait(false);
    //TODO: await GenerateKSailConfigFileAsync($"{clusterName}-ksail-config.yaml");
    await _sopsGenerator.GenerateSOPSConfigAsync(rootDirectory, token).ConfigureAwait(false);
  }

  async Task GenerateFluxKustomizations(string clusterName, string fluxSystemDirectory)
  {
    await GenerateRepositoriesFluxKustomization(fluxSystemDirectory).ConfigureAwait(false);
    await GenerateVariablesFluxKustomization(clusterName, fluxSystemDirectory).ConfigureAwait(false);
    await GenerateInfrastructureFluxKustomization(fluxSystemDirectory).ConfigureAwait(false);
    await GenerateAppsFluxKustomization(fluxSystemDirectory).ConfigureAwait(false);
  }
  ...
  {
    await KubernetesGenerator.GenerateConfigMapAsync(Path.Combine(clusterDirectory, "variables/variables.yaml"), clusterName).ConfigureAwait(false);
    await KubernetesGenerator.GenerateSecretAsync(Path.Combine(clusterDirectory, "variables/variables-sensitive.sops.yaml")).ConfigureAwait(false);
  }
  ...
  async Task GenerateInfrastructure(string manifestsDirectory)
  {
    ...
    await _kubernetesGenerator.GenerateNamespaceAsync(Path.Combine(manifestsDirectory, "infrastructure/cert-manager/namespace.yaml"), "cert-manager").ConfigureAwait(false);
    ...
    await _kubernetesGenerator.GenerateNamespaceAsync(Path.Combine(manifestsDirectory, "infrastructure/traefik/namespace.yaml"), "traefik").ConfigureAwait(false);
  ...
  async Task GenerateApps(string manifestsDirectory)
  {
    ...
    ], "podinfo").ConfigureAwait(false);
    await _kubernetesGenerator.GenerateNamespaceAsync(Path.Combine(manifestsDirectory, "apps/podinfo/namespace.yaml"), "podinfo").ConfigureAwait(false);
  }
