  ...
  internal async Task GenerateSOPSConfigAsync(string manifestsDirectory, CancellationToken token)
  {
    ...
    foreach (string? cluster in clusters)
    {
      if (string.IsNullOrEmpty(cluster))
        continue;
      var publicKey = await _localSOPSProvisioner.GetPublicKeyAsync(KeyType.Age, cluster, token).ConfigureAwait(false);
      publicKeys.Add(publicKey.result);
    }
    await GenerateSOPSConfigAsync("./.sops.yaml", publicKeys).ConfigureAwait(false);
  }
