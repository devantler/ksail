...
class KSailClusterConfigGenerator
{
  ...
  internal async Task GenerateAsync(string name, KSailKubernetesDistribution distribution, KSailInitTemplate template, string outputPath, CancellationToken cancellationToken)
  {
    var ksailCluster = new KSailCluster()
    {
      ...
      Spec = new KSailClusterSpec()
      {
        ...
        GitOpsTool = template switch
        {
          KSailInitTemplate.FluxDefault => KSailGitOpsTool.Flux,
          KSailInitTemplate.FluxAdvanced => KSailGitOpsTool.Flux,
          _ => throw new NotSupportedException($"The template '{template}' is not supported.")
        },
        Registries =
        [
          new KSailRegistry()
