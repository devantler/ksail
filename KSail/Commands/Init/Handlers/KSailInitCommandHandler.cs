...
class KSailInitCommandHandler(string clusterName, string manifestsDirectory) : IDisposable
{
  ...
  internal async Task<int> HandleAsync(CancellationToken token)
  {
    ...
    if (await ProvisionSOPSKey(clusterName, token).ConfigureAwait(false) != 0)
    {
      return 1;
    }
    await _initFilesGenerator.GenerateInitFiles(clusterName, manifestsDirectory, token).ConfigureAwait(false);
    Console.WriteLine("");
    return 0;
  }
  ...
  {
    var (keyExistsExitCode, keyExists) = await _sopsGenerator.KeyExistsAsync(KeyType.Age, clusterName, token).ConfigureAwait(false);
    ...
    if (!keyExists && await _sopsGenerator.CreateKeyAsync(KeyType.Age, clusterName, token).ConfigureAwait(false) != 0)
    {
