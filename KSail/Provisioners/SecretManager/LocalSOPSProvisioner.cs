...
sealed class LocalSOPSProvisioner() : ISecretManagerProvisioner, IDisposable
{
  ...
  public async Task<int> ProvisionAsync(KeyType keyType, string keyName, string k8sContext, CancellationToken token)
  {
    ...
    {
      case KeyType.Age:
        var (exitCode, result) = await GetPrivateKeyAsync(KeyType.Age, keyName, token).ConfigureAwait(false);
        if (exitCode != 0)
        {
          return 1;
        }
        ...
        {
          ["sops.agekey"] = result
        }, "flux-system").ConfigureAwait(false);
        return 0;
      default:
  ...
  public Task<int> CreateKeyAsync(KeyType keyType, string keyName, CancellationToken token) => throw new NotImplementedException();
  ...
  public async Task<(int exitCode, string result)> GetPublicKeyAsync(KeyType keyType, string keyName, CancellationToken token)
  {
    ...
    {
      case KeyType.Age:
        string publicKeyLine = (await File.ReadAllLinesAsync($"{Environment.GetFolderPath(Environment.SpecialFolder.UserProfile)}/.ksail/age/{keyName}.agekey", token).ConfigureAwait(false)).Skip(1).First();
  ...
  public async Task<(int exitCode, string result)> GetPrivateKeyAsync(KeyType keyType, string keyName, CancellationToken token)
  {
    ...
    {
      case KeyType.Age:
        string privateKey = (await File.ReadAllLinesAsync($"{Environment.GetFolderPath(Environment.SpecialFolder.UserProfile)}/.ksail/age/{keyName}.agekey", token).ConfigureAwait(false))[^1];
        return (0, privateKey);
      default:
        throw new NotSupportedException($"🚨 Unsupported key type '{keyType}'");
    }
