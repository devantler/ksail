...
class KubeconformCLIWrapper()
{
  ...
  internal static async Task<int> RunAsync(string[] kubeconformFlags, string[] kubeconformConfig, string manifest, CancellationToken cancellationToken)
  {
    var arguments = kubeconformFlags.Concat(kubeconformConfig).Concat([manifest]);
    var cmd = Kubeconform.WithArguments(arguments);
    var (exitCode, _) = await CLI.RunAsync(cmd, silent: true, cancellationToken: cancellationToken).ConfigureAwait(false);
    return exitCode;
  }
}
