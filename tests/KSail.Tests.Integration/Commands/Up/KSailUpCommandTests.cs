...
public class KSailUpCommandTests : IAsyncLifetime
{
  ...
  public async Task KSailUpEnv_SucceedsAndCreatesCluster()
  {
    ...
    //Act
    if (!File.Exists(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".ksail", "age", "ksail-up.agekey")))
    {
      _ = await ksailSOPSCommand.InvokeAsync("--generate-key");
    }
    string key = await File.ReadAllTextAsync(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".ksail", "age", "ksail-up.agekey"));
