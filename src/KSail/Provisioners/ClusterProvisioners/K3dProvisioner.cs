...
public class K3dProvisioner() : IClusterProvisioner
{
  ...
  {
    await foreach (
      var cmdEvent in !string.IsNullOrEmpty(configPath) ?
        K3dCLIWrapper.K3d.WithArguments($"cluster create --config {configPath}").ListenAsync() :
        K3dCLIWrapper.K3d.WithArguments($"cluster create {name}").ListenAsync()
    )
    {
  ...
  {
    await foreach (
      var cmdEvent in K3dCLIWrapper.K3d.WithArguments($"cluster delete {name}").ListenAsync()
    )
    {
      Console.WriteLine(cmdEvent);
    }
